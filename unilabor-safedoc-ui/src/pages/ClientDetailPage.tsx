...
import {
  ...
  type ClientDocumentUploadSubmitPayload,
} from '../components/clients/ClientDocumentUploadModal';
import { ClientDocumentHistoryModal } from '../components/clients/ClientDocumentHistoryModal';
...
export const ClientDetailPage = () => {
  ...
  const canWrite = useHasPermission('PROVIDERS.CLIENTS.DOCUMENTS.WRITE');

  const [client, setClient] = useState<ClientSummary | null>(null);
  const [documents, setDocuments] = useState<ClientDocument[]>([]);
  const [categories, setCategories] = useState<ClientDocumentCategory[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [documentsResponse, categoriesResponse] = await Promise.all([
        listClientDocuments(clientId, { all: true }),
        listClientDocumentCategories(),
      ]);
      setClient(documentsResponse.client);
      setDocuments(documentsResponse.documents);
      setCategories(categoriesResponse);
    } catch (error) {
      notifyError(getApiErrorMessage(error, 'No se pudo cargar la ficha del cliente.'));
    } finally {
      setLoading(false);
    }
  }, [clientId]);
  ...
  const openUpload = (currentDocument: ClientDocument | null, mode: 'create' | 'replace' | 'edit' = currentDocument ? 'replace' : 'create') => {
  ...
  const handleUploadSubmit = async (payload: ClientDocumentUploadSubmitPayload) => {
    const { currentDocument, mode } = uploadState;
    if (mode === 'edit' && currentDocument) {
      setSaving(true);
      try {
        await updateClientDocument(currentDocument.id, {
    ...
    try {
      if (currentDocument) {
        ...
        await replaceClientDocument(currentDocument.id, {
          category_id: payload.category_id,
          title: payload.title,
        ...
        await uploadClientDocument(clientId, {
          category_id: payload.category_id,
          title: payload.title,
          description: payload.description,
          document_date: payload.document_date || null,
  ...
  const handleRestore = async (document: ClientDocument) => {
    ...
    try {
      await restoreClientDocument(document.id);
  ...
  const openHistory = async (document: ClientDocument) => {
    ...
    try {
      const { history } = await getClientDocument(document.id);
  ...
  const handleView = async (document: ClientDocument) => {
    try {
      const url = await getClientDocumentBlobUrl(document.id);
  ...
  const handleDeactivate = async (document: ClientDocument) => {
    const confirmed = await confirmAction(
    ...
    try {
      await deactivateClientDocument(document.id);
  ...
  const handleDelete = async (document: ClientDocument) => {
    ...
    try {
      const result = await deleteClientDocument(document.id);
  ...
  if (!clientId) {
    return <div className="text-sm text-[var(--unilabor-neutral)]">Cliente inválido.</div>;
  }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--unilabor-neutral)]">
            ...
            {client?.contact ? ` · Contacto: ${client.contact}` : ''}
          </p>
        </div>
