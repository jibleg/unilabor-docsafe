...
import type { ClientNotificationRecipient, LinkableUser } from '../types/models';
import { notifyError, notifySuccess, notifyWarning } from '../utils/notify';
import { confirmAction } from '../utils/confirm';
import { SearchableSelect } from '../components/SearchableSelect';

export const ClientConfigPage = () => {
  const [recipients, setRecipients] = useState<ClientNotificationRecipient[]>([]);
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await addClientNotificationRecipient(selectedUserId);
  ...
  const handleRemove = async (recipient: ClientNotificationRecipient) => {
    const confirmed = await confirmAction(
      'Quitar destinatario',
      `${recipient.full_name || recipient.email || 'Este usuario'} dejará de recibir alertas de vencimiento de documentos de cliente.`,
      'Quitar',
      'primary',
    );
    if (!confirmed) {
    ...
    try {
      await removeClientNotificationRecipient(recipient.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-brand-700)]">Alertas de vencimiento (Clientes)</h1>
          <p className="text-sm text-[var(--unilabor-neutral)]">
            Usuarios que reciben aviso por correo cuando un documento de cliente está por vencer.
          </p>
        </div>
        <button
          type="button"
