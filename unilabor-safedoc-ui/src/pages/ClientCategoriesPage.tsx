...
import type { ClientDocumentCategory } from '../types/models';
...
export const ClientCategoriesPage = () => {
  ...
  const [editingCategory, setEditingCategory] = useState<ClientDocumentCategory | null>(null);
  ...
  const loadCategories = useCallback(async () => {
    setLoading(true);
    try {
      const data = await listClientDocumentCategories(true);
      setCategories(data);
    } catch (error) {
  ...
  const openEditModal = (category: ClientDocumentCategory) => {
  ...
  const submitForm = async () => {
    ...
    try {
      if (editingCategory) {
        await updateClientDocumentCategory(editingCategory.id, payload);
        notifySuccess('Categoría actualizada correctamente.');
      } else {
        await createClientDocumentCategory(payload);
  ...
  const handleDeactivate = async (category: ClientDocumentCategory) => {
    ...
    try {
      await deactivateClientDocumentCategory(category.id);
  ...
  const handleDelete = async (category: ClientDocumentCategory) => {
    const confirmed = await confirmAction(
    ...
    setBusyId(category.id);
    try {
      await deleteClientDocumentCategory(category.id);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--unilabor-neutral)]">
            Clasificación de los documentos de cliente (contratos, pólizas, convenios, etc.).
          </p>
        </div>
