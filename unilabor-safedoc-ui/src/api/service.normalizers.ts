...
export const normalizeHelpdeskAsset = (input: unknown): HelpdeskAsset | null => {
  ...
  return {
    ...
    review_status: getString(source, ['review_status', 'reviewStatus']) === 'REVIEWED' ? 'REVIEWED' : 'PENDING',
    reviewed_at: getString(source, ['reviewed_at', 'reviewedAt']) || null,
    reviewed_by: getString(source, ['reviewed_by', 'reviewedBy']) || null,
    reviewed_by_name: getString(source, ['reviewed_by_name', 'reviewedByName']) || null,
    is_active: getBoolean(source, ['is_active', 'isActive'], true),
    created_at: getString(source, ['created_at', 'createdAt']),
    updated_at: getString(source, ['updated_at', 'updatedAt']),
