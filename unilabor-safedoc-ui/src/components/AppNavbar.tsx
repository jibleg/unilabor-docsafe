    ...
    <header className="sticky top-0 z-40 border-b border-[rgba(0,65,106,0.08)] bg-white/90 backdrop-blur-xl lg:hidden">
      <div className="navbar px-4">
        <div className="navbar-start">
          <NavLink to={moduleCode === 'RH' ? '/rh' : moduleCode === 'HELPDESK' ? '/helpdesk/dashboard' : moduleCode === 'ADMIN' ? '/admin' : moduleCode === 'PROVIDERS' ? '/providers' : '/quality/dashboard'} className="btn btn-ghost px-2 normal-case">
            ...
            <span className="ml-2 text-base font-bold tracking-tight text-[var(--color-brand-700)]">
              {moduleCode === 'RH' ? 'SafeDoc RH' : moduleCode === 'HELPDESK' ? 'SafeDoc Activos' : moduleCode === 'ADMIN' ? 'SafeDoc Admin' : moduleCode === 'PROVIDERS' ? 'SafeDoc Proveedores' : 'SafeDoc'}
            </span>
          </NavLink>
        </div>
