-- ============================================================================
-- MIGRACION MANUAL PARA pgAdmin — Deploy 2026-07-22 (Sala de Lectura, modulo Calidad)
-- Lectura obligatoria y firma autografa de los documentos del SGC.
-- Crea 2 tablas nuevas + 2 permisos + 2 roles RBAC. 100% ADITIVA: no toca
-- ninguna tabla existente ni ningun dato vivo. Segura sobre los 653 activos y
-- los expedientes reales que ya estan en produccion.
-- ============================================================================
-- Replica lo que hace el runner (node dist/scripts/migrate.js migrate): aplica
--   la migracion 20260722_01 y la registra en public.schema_migrations con SU
--   checksum SHA-256 (8de8bb15b729194a8338c08ae4194928be186834b7f1219c9b1458acc5fbff75).
-- Idempotente: correr el script dos veces no rompe nada (CREATE ... IF NOT EXISTS,
--   inserts con guardas / ON CONFLICT DO NOTHING).
--
-- ⚠️ ORDEN DE DEPLOY: puede aplicarse ANTES o DESPUES de subir el codigo (es
--   puramente aditiva; el codigo viejo ignora las tablas nuevas, y el codigo
--   nuevo responde 503 en la sala de lectura mientras las tablas no existan).
--   Recomendado: aplicar el SQL primero, luego scp dist + pm2 restart.
--
-- NOTA: este deploy NO requiere npm ci (sin dependencias nuevas).
-- ============================================================================
...
VALUES ('20260722_01_quality_reading_room.sql', '8de8bb15b729194a8338c08ae4194928be186834b7f1219c9b1458acc5fbff75')
ON CONFLICT (filename) DO NOTHING;

-- =============================================================================
-- Verificacion (ejecutar tras el COMMIT):
--   SELECT to_regclass('public.quality_reading_publications');      -- no NULL
--   SELECT to_regclass('public.quality_reading_acknowledgements');  -- no NULL
--   SELECT code FROM public.permissions
--     WHERE code IN ('QUALITY.READING.MANAGE','QUALITY.SELF.READING');
--   SELECT code FROM public.roles
--     WHERE code IN ('QUALITY_READING_MANAGER','QUALITY_READER');
--   SELECT checksum FROM public.schema_migrations
--     WHERE filename = '20260722_01_quality_reading_room.sql';
--
-- Tras el deploy, en la UI de Administracion -> Roles y permisos:
--   * QUALITY_READING_MANAGER a quien vaya a publicar lecturas (Calidad o RH).
--   * QUALITY_READER a los colaboradores que solo deben leer y firmar.
-- =============================================================================
