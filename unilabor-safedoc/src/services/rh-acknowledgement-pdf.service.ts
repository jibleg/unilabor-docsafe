 ...
 */
const winAnsiSafe = (value: string): string =>
  Array.from(value)
    .map((char) => {
...
export const buildSignedAcknowledgementPdf = async (
  ...
  drawLabelValue(page, cursor, 'IDENTIFICADOR', `Expediente #${input.documentId}`, fonts);
  drawLabelValue(page, cursor, 'HUELLA SHA-256 DEL DOCUMENTO LEÍDO', input.sourceSha256, fonts);
  drawLabelValue(
    page,
    cursor,
    'COLABORADOR',
    input.employeeCode ? `${input.employeeName} (${input.employeeCode})` : input.employeeName,
    fonts,
  );
  ...
  cursor.y -= 13;
  page.drawText(winAnsiSafe(input.employeeName), {
    x: MARGIN,
    y: cursor.y,
    size: 9,
    font: bold,
