-- ============================================================================
-- MIGRACION MANUAL PARA pgAdmin — Deploy 2026-07-21 (RH: Acuse de lectura + firma)
-- Modulo RH de acuse de lectura y firma autografa de documentos institucionales.
-- Crea 2 tablas nuevas + permisos RBAC. 100% ADITIVA: no toca ninguna tabla
-- existente, es segura sobre la BD viva con datos reales.
-- ============================================================================
-- Replica lo que hace el runner (node dist/scripts/migrate.js migrate): aplica
--   la migracion 20260721_01 y la registra en public.schema_migrations con SU
--   checksum SHA-256 (9436cfd048115a2f83b70dbf251a84a6433e60e89d35842d2f7661ec9906df9f).
...
--   puramente aditiva; el codigo viejo ignora las tablas nuevas). Recomendado:
--   aplicar el SQL primero, luego scp dist + npm ci + pm2 restart.
-- ============================================================================
...
-- =============================================================================
-- 20260721_01_rh_document_acknowledgements.sql
-- =============================================================================
BEGIN;
...
VALUES ('20260721_01_rh_document_acknowledgements.sql', '9436cfd048115a2f83b70dbf251a84a6433e60e89d35842d2f7661ec9906df9f')
ON CONFLICT (filename) DO NOTHING;

-- =============================================================================
-- Verificacion (ejecutar tras el COMMIT):
--   SELECT to_regclass('public.rh_institutional_documents');       -- no NULL
--   SELECT to_regclass('public.rh_document_acknowledgements');     -- no NULL
--   SELECT code FROM public.permissions
--     WHERE code IN ('RH.ACKNOWLEDGEMENTS.MANAGE','RH.SELF.ACKNOWLEDGEMENTS');
--   SELECT checksum FROM public.schema_migrations
--     WHERE filename = '20260721_01_rh_document_acknowledgements.sql';
-- =============================================================================
