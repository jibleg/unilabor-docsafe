...
| 2026-06-12 | Bug corregido: pagina en blanco al guardar plan de mantenimiento | `completada` | Causa raiz: los servicios Helpdesk serializaban columnas `date`/`timestamptz` con `String()`, produciendo formato locale ("Fri Jun 12 2026...") en vez de ISO; `formatDate` del frontend hacia `slice(0,10)` -> Invalid Date -> crash de render. Fix: util `toIsoDate`/`toIsoDateTime` aplicada en maintenance, asset y ticket services; `formatDate`/`formatDateTime` endurecidos contra fechas invalidas. Verificado por API (fechas ISO) y builds OK. |
